...
const crearCajaMenorValidator = [
  body('asignado_a')
    .notEmpty().withMessage('El usuario asignado es requerido')
    .isInt({ min: 1 }).withMessage('ID de usuario inválido'),

  body('saldo_inicial')
    .notEmpty().withMessage('El saldo inicial es requerido')
    .isFloat({ min: 0 }).withMessage('El saldo inicial no puede ser negativo'),

  body('caja_anterior_id')
    .optional({ nullable: true })
    .isInt({ min: 1 }).withMessage('ID de caja anterior inválido'),
