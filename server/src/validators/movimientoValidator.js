...
const crearMovimientoValidator = [
  body('caja_menor_id')
    .notEmpty()
    .withMessage('La caja menor es requerida')
    .isInt({ min: 1 })
  ...
  body('tipo_movimiento')
    ...
    .isIn(['ingreso', 'egreso'])
    .withMessage('Tipo inválido. Opciones: ingreso, egreso'),

  body('concepto')
    .trim()
    .notEmpty()
    .withMessage('El concepto es requerido')
    .isIn(TODOS_CONCEPTOS)
  ...
  body('concepto_otro')
    ...
    .trim()
    .isLength({ max: 100 })
    .withMessage('La descripción del concepto no puede exceder 100 caracteres')
    .custom((value, { req }) => {
...
const actualizarMovimientoValidator = [
  ...
  body('concepto').optional().trim().isIn(TODOS_CONCEPTOS).withMessage('Concepto no válido'),
  ...
  body('valor').optional().isFloat({ min: 0.01 }).withMessage('El valor debe ser mayor a cero'),
...
const aprobarMovimientoValidator = [
  ...
  body('aprobado')
    .notEmpty()
    .withMessage('El estado de aprobación es requerido')
    .isBoolean()
    .withMessage('aprobado debe ser verdadero o falso'),

  body('valor_aprobado')
    .optional({ nullable: true })
    .isFloat({ min: 0 })
    .withMessage('El valor aprobado debe ser un número positivo'),
