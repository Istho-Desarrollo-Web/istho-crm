...
const crearViajeValidator = [
  ...
  body('origen')
    .optional()
    .trim()
    .isLength({ max: 100 }).withMessage('El origen no puede exceder 100 caracteres'),
  ...
  body('cliente_nombre')
    .optional()
    .trim()
    .isLength({ max: 200 }).withMessage('El nombre del cliente no puede exceder 200 caracteres'),

  body('documento_cliente')
    .optional()
    .trim()
    .isLength({ max: 50 }).withMessage('El documento no puede exceder 50 caracteres'),
  ...
  body('valor_descargue')
    .optional({ nullable: true })
    .isFloat({ min: 0 }).withMessage('El valor de descargue debe ser un número positivo'),

  body('num_personas')
    .optional({ nullable: true })
    .isInt({ min: 0 }).withMessage('El número de personas debe ser un entero positivo'),

  body('no_factura')
    .optional()
    .trim()
    .isLength({ max: 50 }).withMessage('El número de factura no puede exceder 50 caracteres'),

  body('valor_viaje')
    .optional({ nullable: true })
    .isFloat({ min: 0 }).withMessage('El valor del viaje debe ser un número positivo'),
...
const actualizarViajeValidator = [
  ...
  body('facturado')
    .optional()
    .isBoolean().withMessage('facturado debe ser verdadero o falso'),
  ...
  body('estado')
    .optional()
    .isIn(['activo', 'completado', 'anulado']).withMessage('Estado no válido'),
