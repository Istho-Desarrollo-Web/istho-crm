...
const crearVehiculoValidator = [
  ...
  body('tipo_vehiculo')
    ...
    .isIn(TIPOS_VEHICULO)
    .withMessage(`Tipo inválido. Opciones: ${TIPOS_VEHICULO.join(', ')}`),
  ...
  body('modelo')
    ...
    .trim()
    .isLength({ max: 10 })
    .withMessage('El modelo no puede exceder 10 caracteres'),
  ...
  body('poliza_responsabilidad')
    ...
    .trim()
    .isLength({ max: 50 })
    .withMessage('La póliza no puede exceder 50 caracteres'),

  body('numero_motor')
    .optional()
    .trim()
    .isLength({ max: 50 })
    .withMessage('El número de motor no puede exceder 50 caracteres'),

  body('numero_chasis')
    .optional()
    .trim()
    .isLength({ max: 50 })
    .withMessage('El número de chasis no puede exceder 50 caracteres'),
...
const actualizarVehiculoValidator = [
  ...
  body('estado')
    .optional()
    .isIn(ESTADOS_VEHICULO)
    .withMessage(`Estado inválido. Opciones: ${ESTADOS_VEHICULO.join(', ')}`),
