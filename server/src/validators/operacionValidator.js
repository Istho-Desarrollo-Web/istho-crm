...
const crearOperacionValidator = [
  body('tipo')
    ...
    .isIn(['ingreso', 'salida'])
    .withMessage('Tipo debe ser "ingreso" o "salida"'),
  ...
  body('documento_wms')
    ...
    .trim()
    .isLength({ max: 50 })
    .withMessage('El documento WMS no puede exceder 50 caracteres'),
  ...
  body('fecha_operacion').optional().isISO8601().withMessage('Fecha de operación inválida'),
  ...
  body('conductor_nombre')
    ...
    .trim()
    .isLength({ max: 150 })
    .withMessage('El nombre del conductor no puede exceder 150 caracteres'),
  ...
  body('detalles').optional().isArray().withMessage('Los detalles deben ser un array'),
...
const actualizarTransporteValidator = [
  param('id').isInt({ min: 1 }).withMessage('ID de operación inválido'),
...
const registrarAveriaValidator = [
  ...
  body('detalle_id').optional().isInt({ min: 1 }).withMessage('ID de detalle inválido'),
  ...
  body('tipo_averia')
    ...
    .trim()
    .isLength({ max: 100 })
    .withMessage('El tipo de avería no puede exceder 100 caracteres'),
...
const cerrarOperacionValidator = [
  ...
  body('enviar_correo')
    .optional()
    .isBoolean()
    .withMessage('enviar_correo debe ser verdadero o falso'),

  body('correos_destino').optional().trim(),
...
const listarOperacionesValidator = [
  ...
  query('estado')
    .optional()
    .isIn(['pendiente', 'en_proceso', 'cerrado', 'anulado', 'todos'])
