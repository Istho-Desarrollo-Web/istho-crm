...
const PERMISOS_CLIENTE_CATALOGO = {
  inventario: [
    ...
    },
    {
      ...
      descripcion: 'Permite exportar el inventario a Excel',
    },
    {
      codigo: 'alertas',
...
module.exports = (sequelize) => {
    ...
    'Usuario',
    {
      ...
      username: {
        ...
        validate: {
          ...
          is: {
            args: /^[A-Za-z0-9._-]+$/,
            msg: 'El username solo puede contener letras, números, puntos, guiones y guiones bajos',
          },
        },
      },
      ...
      preferencias: {
        ...
        defaultValue: null,
        comment:
          'Preferencias del usuario: tema, notificaciones, zona horaria, formato fecha, sesión',
      },
      ...
      permisos_personalizados: {
        ...
        defaultValue: null,
        comment:
          'Permisos personalizados por usuario (override de rol). Array de permiso_ids o null para usar permisos del rol',
        get() {
      ...
      dispositivos_confiables: {
        ...
        defaultValue: null,
        comment:
          'Dispositivos de confianza para omitir 2FA. Array de { id, token_hash, nombre, creado_en, expira_en }',
        get() {
  ...
  Usuario.prototype.toPublicJSON = function (permisosDB) {
    return {
      ...
      password_vencida: !!(
        this.password_expira_en && new Date(this.password_expira_en) < new Date()
      ),
  ...
  Usuario.findByEmailOrUsername = async function (identificador) {
    return this.findOne({
      where: {
        [Op.or]: [{ email: identificador.toLowerCase() }, { username: identificador }],
      },
    });
  };
  ...
  Usuario.crearUsuarioCliente = async function (data, invitadoPorId) {
    const { cliente_id, nombre_completo, email, password, telefono, cargo, permisos_cliente } =
      data;
