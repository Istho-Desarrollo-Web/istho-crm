...
const reservarStock = async (
  ...
  const preloadedInventarios = orConditions.length
  ...
  for (const detalle of detalles) {
    try {
      ...
      if (detalle.producto_id) {
        inventario =
          invById.get(detalle.producto_id) ||
 ...
 * @param {Transaction} transaction - Transacción de Sequelize
 */
const liberarStockReservado = async (
  operacionId,
  numeroOperacion,
  usuarioId,
  ipAddress,
...
const confirmarMovimientoStock = async (operacion, usuarioId, ipAddress, transaction) => {
  ...
  for (const detalle of detalles) {
    try {
      ...
      if (detalle.inventario_id) {
        inventario =
          invById.get(detalle.inventario_id) ||
          (await Inventario.findByPk(detalle.inventario_id, {
            transaction,
            lock: transaction.LOCK.UPDATE,
          }));
      } else if (detalle.sku) {
...
const crear = async (req, res) => {
  ...
  try {
    ...
    if (datos.documento_wms) {
      ...
      for (let i = 0; i < datos.detalles.length; i++) {
        ...
        if (!det.cantidad || det.cantidad <= 0) {
          ...
          return errorResponse(
            res,
            `El producto en posición ${i + 1} debe tener cantidad mayor a 0`,
            400
          );
        }
      }
      ...
      datos.total_unidades = datos.detalles.reduce(
        (sum, d) => sum + (parseFloat(d.cantidad) || 0),
        0
      );
...
const registrarAveria = async (req, res) => {
  ...
  try {
    ...
    if (datos.detalle_id) {
      ...
      if (detalle) {
        const nuevaCantidadAveria =
          parseFloat(detalle.cantidad_averia) + parseFloat(datos.cantidad);
...
const cerrar = async (req, res) => {
  ...
  try {
    ...
    notificacionService
      ...
      .catch((err) => {
        logger.error('Error al enviar notificación de cierre:', {
          message: err.message,
          operacion_id: operacion.id,
          cliente_id: operacion.cliente_id,
        });
...
const reenviarCorreo = async (req, res) => {
  try {
    ...
    if (!correosEnvio) {
      return errorResponse(
        res,
        'No hay destinatarios configurados. Verifique los contactos del cliente.',
        400
      );
    }
    ...
    return successMessage(
      res,
      resultado.success ? 'Correo reenviado exitosamente' : 'Error al reenviar correo',
      {
        correo_enviado: resultado.success,
        correos_destino: correosEnvio,
      }
    ...
    if (averia.detalle_id) {
      ...
      if (detalle) {
        const nuevaCantidad = Math.max(
          0,
          parseFloat(detalle.cantidad_averia) - parseFloat(averia.cantidad)
        );
