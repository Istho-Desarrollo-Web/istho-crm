...
import { Smartphone, ShieldCheck, ShieldOff, Copy, Eye, EyeOff, CheckCircle2, AlertCircle, Loader2, X, Lock } from 'lucide-react';
...
const Configuracion2FA = () => {
    ...
    return (
      <div className="rounded-2xl border border-green-200 dark:border-green-800/50 bg-green-50 dark:bg-green-900/10 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-base font-semibold text-green-800 dark:text-green-300">¡2FA activado!</h3>
        </div>
        <p className="text-sm text-green-700 dark:text-green-400 mb-4">
          Guarda estos códigos de respaldo en un lugar seguro. Solo se muestran <strong>una vez</strong> y sirven si pierdes acceso a tu autenticador.
        </p>
        <div className="grid grid-cols-2 gap-2 mb-4">
          {backupCodes.map((code, i) => (
            <div key={i} className="bg-white dark:bg-centhrix-card border border-green-200 dark:border-green-800/50 rounded-lg px-3 py-2 text-center font-mono text-sm text-slate-700 dark:text-slate-200 tracking-widest">
              {code}
            </div>
          ))}
        </div>
        <div className="flex gap-3">
          <button onClick={copiarCodigos} className="flex items-center gap-2 text-sm text-green-700 dark:text-green-400 hover:text-green-800 transition-colors">
            <Copy className="w-4 h-4" /> Copiar todos
          </button>
          <button onClick={finalizarSetup} className="ml-auto px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-xl transition-colors">
            Listo
          </button>
        </div>
      </div>
    ...
    return (
      <div className="rounded-2xl border border-orange-200 dark:border-orange-800/50 bg-orange-50 dark:bg-orange-900/10 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => { setPaso(PASOS.IDLE); setSetupData(null); }} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
            <X className="w-5 h-5" />
          </button>
        </div>

        <ol className="text-sm text-slate-600 dark:text-slate-400 space-y-1 mb-4 list-decimal list-inside">
          <li>Abre <strong>Google Authenticator</strong>, <strong>Authy</strong> o similar.</li>
        ...
        {setupData?.qr_code && (
          <div className="flex justify-center mb-4">
            <img src={setupData.qr_code} alt="QR 2FA" className="w-48 h-48 border-4 border-white dark:border-centhrix-card rounded-xl shadow" />
          </div>
        )}
        ...
        {setupData?.secret && (
          <div className="mb-4 bg-white dark:bg-centhrix-card border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
            <span className="font-mono text-xs text-slate-600 dark:text-slate-300 break-all">{setupData.secret}</span>
            <button onClick={() => { navigator.clipboard.writeText(setupData.secret); notifySuccess('Secreto copiado'); }} className="flex-shrink-0 text-slate-400 hover:text-orange-500 transition-colors">
        ...
        <form onSubmit={confirmarActivacion} className="space-y-3">
          ...
          <input
            ...
            autoFocus
            value={codigoConfirm}
            onChange={(e) => { setCodigoConfirm(e.target.value); setErrorConfirm(null); }}
          ...
          >
            {loadingSetup ? <><Loader2 className="w-4 h-4 animate-spin" /> Verificando...</> : 'Activar 2FA'}
          </button>
        </form>
      </div>
    );
  ...
  return (
    <div className={`rounded-2xl border p-6 ${habilitado ? 'border-green-200 dark:border-green-800/50 bg-green-50/50 dark:bg-green-900/5' : 'border-slate-200 dark:border-centhrix-surface bg-white dark:bg-centhrix-card'}`}>
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-slate-800 dark:text-slate-100">Autenticación de dos factores (2FA)</p>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {habilitado ? 'Activo — tu cuenta está protegida con TOTP.' : 'Inactivo — actívalo para mayor seguridad.'}
            </p>
          </div>
        </div>
        ...
        {habilitado ? (
          ...
          >
            {loadingSetup ? <Loader2 className="w-4 h-4 animate-spin" /> : <Smartphone className="w-4 h-4" />}
            Activar 2FA
          </button>
        )}
      </div>
          ...
          <form onSubmit={desactivar} className="space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => { setPasswordDesactivar(e.target.value); setErrorDesactivar(null); }}
              ...
              />
              <button type="button" onClick={() => setShowPwdDesactivar(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors">
                {showPwdDesactivar ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={() => { setShowDesactivar(false); setPasswordDesactivar(''); setErrorDesactivar(null); }} className="flex-1 py-2 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 border border-slate-200 dark:border-slate-700 rounded-xl transition-colors">
                Cancelar
              </button>
              <button type="submit" disabled={loadingDesactivar || !passwordDesactivar} className="flex-1 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:opacity-70 rounded-xl transition-colors flex items-center justify-center gap-1">
                {loadingDesactivar ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Desactivar 2FA'}
              </button>
            </div>
          </form>
        </div>
