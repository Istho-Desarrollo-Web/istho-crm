...
const useNotification = () => {
   ...
   */
  const despachoCerrado = useCallback(
    (numero) => {
   ...
   */
  const inventoryAlert = useCallback(
    (total, porTipo = {}) => {
      if (total === 0) return;
      ...
      const mensaje = partes.length === 1 ? `${partes[0]}` : `Alertas: ${partes.join(', ')}`;
   ...
   */
  const operacionCreada = useCallback(
    (tipo, numero) => {
