...
const useDespachos = (options = {}) => {
  const { autoFetch = false, clienteId = null, initialFilters = {} } = options;
  ...
  const actualizarTransporte = useCallback(async (id, transporteData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setListState((prev) => ({
          ...prev,
          data: prev.data.map((d) => (d.id === id ? { ...d, ...response.data } : d)),
        }));
      } else {
        throw new Error(response.message);
      }
   ...
   */
  const filterByFechas = useCallback(
