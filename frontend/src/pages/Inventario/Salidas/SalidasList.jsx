...
const SalidasList = () => {
  ...
  const handleToggleSelect = (id) => {
    const item = salidas.find((s) => s.id === id);
  ...
  const handleSelectAll = () => {
    if (allCloseableSelected) {
      ...
      const targetCliente =
        selectedCliente || salidas.find((s) => closeableIds.includes(s.id))?.cliente;
      const idsToSelect = closeableIds.filter((id) => {
        const s = salidas.find((sl) => sl.id === id);
        return !targetCliente || s?.cliente === targetCliente;
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950">
      <main className="pt-28 px-4 pb-8 max-w-[1700px] mx-auto">
        ...
        {/* BULK ACTION BAR */}
        {selectedIds.size > 0 && (
          <div className="mb-4 flex items-center justify-between gap-3 px-4 py-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
            <div className="flex items-center gap-2">
              <CheckSquare2 className="w-4 h-4 text-blue-600 dark:text-blue-400" />
              ...
              <button
                onClick={() => {
                  ...
                  [...selectedIds].forEach((id) => {
                    const s = salidas.find((sl) => sl.id === id);
                    map[id] = { origen: s?.origen || '', destino: s?.destino || '' };
                  });
                  setModalCedula('');
                  setModalTelefono('');
                  setModalTipoVehiculo('');
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filtered.map((salida) => (
                    <tr
                      key={salida.id}
                      className={`border-b border-gray-50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-centhrix-surface/30 transition-colors cursor-pointer group ${selectedIds.has(salida.id) ? 'bg-blue-50/40 dark:bg-blue-900/10' : ''}`}
                      onClick={() => handleView(salida)}
                    >
                      <td className="py-4 pl-4 pr-2" onClick={(e) => e.stopPropagation()}>
                        {(salida.estado === 'pendiente' || salida.estado === 'en_proceso') && (
                          <input
                            type="checkbox"
                            checked={selectedIds.has(salida.id)}
                            onChange={() => handleToggleSelect(salida.id)}
                            className="w-4 h-4 rounded border-slate-300 dark:border-slate-600 accent-blue-500 cursor-pointer"
                          />
                        )}
                      </td>
            ...
            <div className="p-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-8 pr-3 py-2 text-sm border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-centhrix-bg text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500"
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    return (
                      <div key={id} className="bg-slate-50 dark:bg-centhrix-bg rounded-xl p-3">
                        <p className="text-xs font-mono font-semibold text-slate-600 dark:text-slate-300 mb-2">
                          {salida.documento_wms || salida.documento || `#${id}`}
                        </p>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                className="w-full pl-7 pr-2 py-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-centhrix-surface text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500"
                              />
                            </div>
                          </div>
                          <div>
              ...
              <div className="mb-5">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-slate-200 dark:border-slate-600 rounded-xl text-sm text-slate-500 dark:text-slate-400 hover:border-blue-400 dark:hover:border-blue-600 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                >
