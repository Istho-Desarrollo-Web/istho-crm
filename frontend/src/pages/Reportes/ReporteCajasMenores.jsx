    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950">
      <main className="pt-28 px-4 pb-8 max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">
                  Reporte de Cajas Menores
                </h1>
                <p className="text-slate-500 dark:text-slate-400 text-sm">
                  Resumen de cajas menores, saldos y movimientos
                </p>
              </div>
            </div>
          </div>
        ...
        <div id="tour-reportes-cajas-menores-kpis" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <KpiCard
            title="Total Egresos"
            value={formatCOP(kpis.totalEgresos)}
            icon={TrendingDown}
          ...
          />
          <KpiCard
            title="Saldo Actual Total"
            value={formatCOP(kpis.totalSaldoActual)}
            icon={DollarSign}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <PieChart
            ...
            subtitle="En cajas activas"
            data={data?.egresosVsIngresos || []}
            size={180}
            loading={loading}
          />
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(data?.ultimas || []).map((c) => (
                  <tr
                    key={c.id}
                    onClick={() => navigate(`/viajes/cajas-menores/${c.id}`)}
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-right text-emerald-600 dark:text-emerald-400">
                      +{formatCOP(c.total_ingresos)}
                    </td>
                    <td className="py-3 px-4 text-right text-red-600 dark:text-red-400">
                      -{formatCOP(c.total_egresos)}
                    </td>
                    <td
                      className={`py-3 px-4 text-right font-bold ${(parseFloat(c.saldo_actual) || 0) >= 0 ? 'text-slate-800 dark:text-white' : 'text-red-600 dark:text-red-400'}`}
                    >
                ...
                {(!data?.ultimas || data.ultimas.length === 0) && !loading && (
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-slate-400">
                      No hay cajas menores registradas
                    </td>
                  </tr>
                )}
              </tbody>
