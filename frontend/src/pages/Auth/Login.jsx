...
const loginSchema = yup.object({
  email: yup.string().required('El usuario o email es requerido'),
  password: yup
    .string()
    .required('La contraseña es requerida')
    .min(6, 'La contraseña debe tener al menos 6 caracteres'),
  aceptaPolitica: yup.boolean().oneOf([true], 'Debes aceptar la política de tratamiento de datos'),
});
...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        setMensajePendiente(JSON.parse(stored));
      } catch {
  ...
  const onSubmit2FA = async (e) => {
    ...
    try {
      const result = await validarTotp(
        paso2FA.temp_token,
        codigoTotp.replace(/\s/g, ''),
        recordarDispositivo
      );
  ...
  if (paso2FA) {
    return (
      <div
        className="h-screen flex items-center justify-center bg-white dark:bg-centhrix-bg"
        style={fadeIn}
      >
        <div className="w-full max-w-sm px-6" style={slideUp}>
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 mt-2 text-center">
              Hola <strong>{paso2FA.usuario_nombre}</strong>. Ingresa el código de tu aplicación
              autenticadora o un código de respaldo.
            </p>
          </div>
          ...
          <form onSubmit={onSubmit2FA} className="space-y-4">
            <div>
              ...
              <input
                ...
                value={codigoTotp}
                onChange={(e) => {
                  setCodigoTotp(e.target.value.toUpperCase());
                  setError2FA(null);
                }}
  ...
  return (
    <>
      ...
      <div
        className="h-screen flex relative bg-white dark:bg-centhrix-bg overflow-hidden"
        style={fadeIn}
      >
        {/* ════════════════════════════════════════════════════════════════════ */}
          ...
          <div className="w-full max-w-md" style={slideUp}>
            ...
            <div className="text-center mb-6">
              <img
                src={logoIstho}
                alt="ISTHO"
                className="h-16 w-auto max-w-[160px] object-contain mb-4 mx-auto drop-shadow-[0_2px_8px_rgba(0,0,0,0.35)] dark:drop-shadow-none dark:brightness-0 dark:invert"
              />
            ...
            {authError && (
              <div
                className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-2xl flex items-start gap-3"
                style={slideUp}
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-red-600 dark:text-red-400 mt-1">{authError}</p>
                </div>
              </div>
            )}
        ...
        <div className="right-panel hidden lg:flex lg:flex-1 bg-gradient-to-br from-[#1A1A2E] via-[#2C3E50] to-[#1A1A2E] dark:from-[#0F1023] dark:via-[#151631] dark:to-[#0F1023] relative overflow-hidden">
          ...
          {/* Contenido */}
          <div
            className="panel-content relative z-10 flex flex-col items-center justify-center p-12 text-white w-full"
            style={fadeIn}
          >
            {/* Logo Centhrix */}
            <div className="mb-8" style={{ animation: 'float 6s ease-in-out infinite' }}>
              <img src={logoCenthrix} alt="CenthriX" className="w-48 h-auto drop-shadow-2xl" />
            </div>

            <h2 className="centhrix-title text-4xl font-bold mb-3 text-center cursor-default select-none">
              CRM Centhri
              <span className="letter-x" style={{ color: '#2ECC71' }}>
                X
              </span>
            </h2>
      ...
      {pasoSetup2FA && (
        ...
        >
          <div
            className="bg-white dark:bg-centhrix-card w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700"
            style={slideUp}
          >
            <div className="p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400">
                    Configura tu autenticador para continuar
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-6" style={fadeIn}>
                  <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl">
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-400">
                      Guarda estos códigos de respaldo. Son la única forma de acceder si pierdes tu
                      celular.
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {backupCodes.map((code, idx) => (
                      <div
                        key={idx}
                        className="bg-slate-50 dark:bg-centhrix-bg border border-slate-200 dark:border-slate-700 rounded-xl py-2 px-3 text-center font-mono text-sm dark:text-slate-200"
                      >
                        {code}
                      </div>
                    ))}
                ...
                <div className="space-y-6">
                  ...
                  <form onSubmit={handleConfirmarSetup} className="space-y-4">
                    <div>
                      ...
                      {errorSetup && (
                        <div
                          className="mt-2 flex items-center gap-2 text-red-500 text-xs font-medium"
                          style={fadeIn}
                        >
